...
function _getNewCard(rank, suit) {
    return (_.isUndefined(rank) || _.isUndefined(suit) || _.isNull(rank) || _.isNull(suit) || !_isValidRank(rank) || !_isValidSuit(suit))
        ? undefined
        : _.i.Map([['rank', rank], ['suit', suit], ['value', _getRankValues(rank)]]);
}

//takes a card tuple [rank, suit] and returns a boolean
function _isValidCard(card) {
    return (card.isMap() && _isValidRank(card.get('rank')) && _isValidSuit(card.get('suit')));
}
...
function _getSuits(){
    return _.i.List(['hearts', 'clubs', 'spades', 'diamonds']);
}
...
function _getRanks() {
    return _.i.List(['a','2','3','4','5','6','7','8','9','10','j','q','k']);
}

function _getRankValues(rank){
    _.i.Map([['a', _.i.List([1, 11])],
        ['2', _.i.List([2, 2])],
        ['3', _.i.List([3, 3])],
        ['4', _.i.List([4, 4])],
        ['5', _.i.List([5, 5])],
